$(function() {
	...
	(function(){
        ...
        //先获取音量控制容器的width
        var $volumeControlWidth = $volumeControl.width();
        //设置默认的音量
        player.volume = 0.8;
        //默认音量bar长度
        $volumeInner.css('width',player.volume * $volumeControlWidth + 'px' );
		...
		var volumeSize; //用于保存禁音前的音量
		$volume.on('click', function() {
			if (player.muted) {
				player.muted = false;
				$(this).removeClass('icon-volume-mute').addClass('icon-volume');
				$volumeInner.css('width', volumeSize);
			} else {
                volumeSize = $volumeInner.css('width');   //获取禁音前的音量
				player.muted = true;
